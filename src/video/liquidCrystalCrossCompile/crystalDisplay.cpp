...
void listenToUDPAndWrite(FILE *stream)
{
	...
	lcd.print("0xDEADC0DE");
	lcd.setCursor(0, 1);
	lcd.print("netcat -u");
	lcd.setCursor(0, 2);
	lcd.print("ENSC351 FALL2022");
	...
	while (1)
	{
		...
		char messageRx[MSG_MAX_LEN*2];
		char messageRxTop[MSG_MAX_LEN];
		char messageRxBottom[MSG_MAX_LEN];

		// Pass buffer size - 1 for max # bytes so room for the null (string data)
		int bytesRx = recvfrom(socketDescriptor,
							   messageRx, (MSG_MAX_LEN*2) - 1, 0,
		...
		messageRx[bytesRx] = 0;
		if (strcmp(messageRx, "clear") == 0){
			lcd.clear();
		} else {
char* e = strchr(messageRx, '+');
		int seperateIdx = (int)(e - messageRx);

		//Get First string
		memcpy(messageRxTop, &(messageRx[0]), seperateIdx);
		messageRxTop[seperateIdx] = 0;
		//printf("sepe: %s, %d", messageRxTop, seperateIdx);

		//Get Second Strng 
		memcpy(messageRxBottom, &(messageRx[seperateIdx+1]), bytesRx - seperateIdx);
		messageRxBottom[bytesRx - seperateIdx] = 0;
		//printf("sepe2: %s, %d", messageRxBottom, bytesRx - seperateIdx);

		printf("[CHILD] Message received (%d bytes): \n\n[CHILD]'%s'\n", bytesRx, messageRx);
		lcd.setCursor(0,0);
		lcd.print(messageRxTop);
		lcd.setCursor(0,1);
		lcd.print(messageRxBottom);
		}
